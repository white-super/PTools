...
<p align="center">
  <a href="#界面预览">界面预览</a> · <a href="#下载与安装">下载与安装</a> · <a href="#主要功能">主要功能</a> · <a href="#快捷工具">快捷工具</a> · <a href="#格式化工具">格式化工具</a> · <a href="#文本对比">文本对比</a> · <a href="#顺序粘贴">顺序粘贴</a> · <a href="#todo">TODO</a> · <a href="#使用方法">使用方法</a> · <a href="CHANGELOG.md">版本历史</a> · <a href="#隐私与权限">隐私与权限</a> · <a href="#开源许可">开源许可</a>
</p>

PTools 是一个本地优先的跨平台剪贴板工具集，将历史记录、搜索、标签、格式化和文本对比集中到复制粘贴场景中。剪贴板数据与工具处理均在本机完成，不会上传内容。

> 当前源码包含正在开发的 `1.4.0` 功能；最新正式版仍为 `1.3.0`。开发进度请查看 [版本历史](CHANGELOG.md)。
...
- **搜索与分类**：搜索完整历史数据，按内容类型或自定义标签筛选；带标签记录不受历史数量和保留期限清理。
- **快速粘贴**：双击、回车或数字键选择记录，将内容恢复到系统剪贴板，并可自动粘贴回原应用。
- **快捷工具栏**：最多放置 5 个常用工具，支持拖拽排序、增删工具和自定义快捷键。
- **格式化与转换**：识别 JSON/JSONC、XML、HTML、URL、Base64 和日期内容，在独立窗口中格式化或转换。
- **文本对比**：对比剪贴板文本或本地 UTF-8 文本文件，并可先格式化 JSON、XML 或 HTML 再计算差异。
- **顺序粘贴**：临时采集多条文本、图片或文件，调整顺序后通过全局快捷键逐项粘贴。
- **可控设置**：可修改主题、快捷键、记录类型、保留时间和历史数量，或一键清空历史。

## 快捷工具

粘贴面板顶部中央提供轻量快捷工具栏，默认显示“文本对比、顺序粘贴、JSON 格式化、URL 编解码、Base64 编解码”。

![PTools 快捷工具栏与管理面板](images/QuickTools.png)

1. 点击田字格入口管理工具，可从文本对比、顺序粘贴、JSON、XML、HTML、URL、Base64 和日期工具中最多选择 5 个。
2. 直接拖动工具可调整顺序；在管理面板中点击已选工具可将其移除，配置会保存在本机。
3. 默认使用 `cmd+1` 至 `cmd+5` 打开对应位置的工具；在粘贴面板内使用快捷键时，格式化工具会导入当前选中的文本卡片。排序后快捷键会自动跟随新位置，也可在设置界面逐项修改。
4. 将鼠标移到图标上可立即查看工具名称和当前快捷键；已经打开的工具会显示激活状态。
5. 直接点击顶部格式化工具图标会打开空白工作区，不会带入剪贴板内容；“文本对比”会打开左右空白、默认置顶的对比窗口，“顺序粘贴”会打开右侧临时队列。
6. 右键点击文本卡片并展开“使用工具”，仍可访问全部文本工具，不受快捷工具栏数量限制。

## 格式化工具

![PTools JSON 格式化工具](images/JsonFormatter.png)

1. 在粘贴板面板中选中文本记录。
2. 按 `F` 打开独立的格式化工具窗口；带转义的 JSON 文本也可以识别。
3. 直接点击顶部快捷工具会打开空白工作区；在卡片上使用对应快捷键，或右键展开“使用工具”，会带入当前卡片内容。
...
7. 编辑器搜索栏显示在顶部并提供中文操作项；第一次按 `Esc` 关闭搜索，再次按 `Esc` 关闭格式化窗口。
8. 使用 `Command + D` 或 `Alt + D` 固定/取消固定窗口；固定窗口不会因失去焦点自动关闭，也可以同时打开多个格式化窗口。

## 文本对比

![PTools 文本对比工具](images/TextDiff.png)
...
3. 点击粘贴面板顶部快捷工具栏的“文本对比”（默认 `cmd+1`），可打开左右均为空的对比窗口并自行粘贴内容；从该入口打开的窗口默认置顶。
...
<p align="center">
  <img src="images/SequentialPaste.png" width="360" alt="PTools 顺序粘贴临时队列" />
</p>

1. 从粘贴面板顶部快捷工具栏打开“顺序粘贴”，屏幕右侧会保留一条独立的紧凑队列；该入口不会导入当前卡片或已有历史。
2. 采集模式会自动收集之后复制的文本、图片和文件，允许重复内容，并支持拖拽排序、单项移除和清空。
3. 切换到粘贴模式后可选择正序或倒序。默认按 `Ctrl + Shift + V` 粘贴下一项，快捷键可在设置中修改。
4. 只有成功写入目标应用的内容才会出队，队列会高亮下一项并保持窗口可见；失败内容会保留并在对应位置显示错误。队列完成后快捷键自动释放。
5. 队列仅存在于本次使用期间；关闭队列窗口或退出 PTools 后会清空，不会写入独立的持久化队列表。
...
- [x] **顺序粘贴**：临时采集多条剪贴板内容，支持排序、正序/倒序和可配置快捷键逐项粘贴。
- [ ] **cURL 请求工具**：解析 cURL 文本并在独立请求工作区中发送和查看响应。
- [ ] **图片裁剪工具**：对图片剪贴板内容进行裁剪、缩放和复制或保存。

以上是 `1.4.0` 的开发计划；已勾选功能已进入当前源码，未勾选功能尚未完成，具体交互和快捷键仍可能调整。

## 使用方法

1. 复制文本、图片或文件，PTools 会按当前设置在本机记录它。
2. macOS 按 `Ctrl + V`、Windows 按 `Alt + V` 打开面板；可在设置中修改为自己的组合键。
3. 通过搜索、内容类型或标签筛选记录，单击选中内容；macOS 可按 `Command + F` 快速定位搜索框。
4. 双击记录、按回车或按数字 `1`–`5` 恢复内容。
5. 使用 `F`、`D`、顶部快捷工具栏或卡片右键菜单处理当前内容；需要连续录入时可打开“顺序粘贴”。
6. 若开启“**双击后自动粘贴**”，PTools 会将内容粘贴回原应用；否则只恢复到系统剪贴板。
